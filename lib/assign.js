...
function map2Right( x, y, out, fcn, thisArg ) {
	...
	var sh;
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '1VV32', fcn ) );
	}
	...
	if ( isxnd ) { // note: assertion order matters here, as an ndarray-like object is also array-like
		if ( !isynd ) {
			throw new TypeError( format( '1VVBI', y ) );
		}
		if ( !isznd ) {
			throw new TypeError( format( '1VVBJ', out ) );
		}
		if ( isReadOnly( out ) ) {
			throw new Error( format('1VV1g') );
		}
